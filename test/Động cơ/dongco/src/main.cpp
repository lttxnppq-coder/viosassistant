...
void setup() {
    ...
    // Using modern Arduino-ESP32 API: ledcAttach(pin, freq, resolution)
    bool in1Ok = ledcAttach(IN1_PIN, PWM_FREQ_HZ, PWM_RES_BITS);
    bool in2Ok = ledcAttach(IN2_PIN, PWM_FREQ_HZ, PWM_RES_BITS);
    
    if (!in1Ok || !in2Ok) {
        Serial.println(F("[ERROR] Failed to attach PWM channels!"));
        while (true) { delay(1000); }
    }
    
    // Ensure PWM outputs start at 0 duty
    ledcWrite(IN1_PIN, 0);
    ledcWrite(IN2_PIN, 0);
...
void motorForward(int speed) {
    ...
    ledcWrite(IN2_PIN, 0);
    // Ensure IN2 is driven LOW (not PWM 0 which might have glitches)
    // ledcWrite with 0 duty should hold LOW, but explicit digitalWrite helps
    pinMode(IN2_PIN, OUTPUT);
    digitalWrite(IN2_PIN, LOW);
    // Restore PWM on IN1
    ledcAttach(IN1_PIN, PWM_FREQ_HZ, PWM_RES_BITS);
...
void motorBackward(int speed) {
    ...
    digitalWrite(IN1_PIN, LOW);
    // Restore PWM on IN2
    ledcAttach(IN2_PIN, PWM_FREQ_HZ, PWM_RES_BITS);
